...
def main():
    ...
    for thisMonkey in monkeys:
        thisMonkey.items = [[item % monkeys[i].divisor for i in range(8)] 
                            for item in thisMonkey.items]
    
    for roundnb in range(nbRounds):
        for monkeyNb, thisMonkey in enumerate(monkeys):
            for item in thisMonkey.items:
                if thisMonkey.operation == '+':
                    item = [((remainder + thisMonkey.operand) % monkeys[i].divisor) 
                            for i, remainder in enumerate(item)]
                elif thisMonkey.operation == '*':
                    item = [((remainder * thisMonkey.operand) % monkeys[i].divisor) 
                            for i, remainder in enumerate(item)]
                elif thisMonkey.operation == '^':
                    item = [((remainder * remainder) % monkeys[i].divisor) 
                            for i, remainder in enumerate(item)]
